...
import com.github.javaparser_new.ast.nodeTypes.NodeWithMembers;
import com.github.javaparser_new.ast.nodeTypes.NodeWithSimpleName;
...
public class JavaParserTypeAdapter<T extends Node & NodeWithSimpleName<T> & NodeWithMembers<T>> {
            ...
            if (member instanceof TypeDeclaration) {
                ...
                if (internalType.getName().getId().equals(name)) {
                    if (internalType instanceof ClassOrInterfaceDeclaration) {
                        ...
                        return SymbolReference.solved(new JavaParserAnnotationDeclaration((AnnotationDeclaration) internalType, typeSolver));
                    } else {
                        throw new UnsupportedOperationException();
                    }
                    ...
                    if (internalType instanceof ClassOrInterfaceDeclaration) {
                        if (((ClassOrInterfaceDeclaration) internalType).isInterface()) {
                            return new JavaParserInterfaceDeclaration((ClassOrInterfaceDeclaration) internalType, typeSolver).solveType(name.substring(prefix.length()));
                        } else {
                            return new JavaParserClassDeclaration((ClassOrInterfaceDeclaration) internalType, typeSolver).solveType(name.substring(prefix.length()));
                        }
                    } else if (internalType instanceof EnumDeclaration) {
    ...
    public List<ResolvedFieldDeclaration> getFieldsForDeclaredVariables() {
        ...
        if (wrappedNode.getMembers() != null) {
            for (BodyDeclaration<?> member : this.wrappedNode.getMembers()) {
                if (member instanceof com.github.javaparser_new.ast.body.FieldDeclaration) {
                    com.github.javaparser_new.ast.body.FieldDeclaration field = (com.github.javaparser_new.ast.body.FieldDeclaration) member;
